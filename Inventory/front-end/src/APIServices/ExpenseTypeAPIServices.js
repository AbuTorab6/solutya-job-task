...
var expenseTypeList = (pageNo,perPage,searchKey)=>
{
    //console.log(baseURL+'/productList/'+pageNo+'/'+perPage+'/'+searchKey);
    return axios.get(baseURL+'/expenseTypeList/'+pageNo+'/'+perPage+'/'+searchKey,axiosHeader).then
    (
        (res)=>
        {
            if(res.status===200)
            ...
            {
                cogoToast.warn("something is wrong.Can not display the expense type list.");
                return false
            }
        }
    ).catch
...
var deleteExpenseType = (id)=>
{
    store.dispatch(showLoader())
    return axios.get(baseURL+'/deleteExpenseType/'+id,axiosHeader).then
    (
        (res)=>
        {
            store.dispatch(hideLoader())
            ...
            {
                cogoToast.warn("can not delete expense type");
                return false
            }
        }
    ).catch
...
export {expenseTypeList,createExpenseType,expenseTypeDetailById,updateExpenseType,deleteExpenseType};